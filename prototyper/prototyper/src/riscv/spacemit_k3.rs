...
//! A100 AI cores).
//!
//! Register definitions and the initialization flow mirror the official
//! OpenSBI implementation (branch `k3-br-v1.0.y`, commit `4869910`):
//! `platform/generic/spacemit/spacemit_k3.c`,
//! `platform/generic/include/spacemit/k3/{k3.h,core_common.h}`,
//! `platform/generic/spacemit/k3_corepm.c` and
...
/// Cluster 2 warmboot address (OpenSBI k3.h `C2_RVBADDR_LO/HI_ADDR`).
const C2_RVBADDR: *const WarmbootAddr = (0xd4282c00usize + 0x3e8) as *const WarmbootAddr;
/// Cluster 3 warmboot address (OpenSBI k3.h `C3_RVBADDR_LO/HI_ADDR`).
...
fn boot_entry_dummy() -> ! {
    ...
    unsafe {
        ...
        // Disable prefetch, flush dcache, disable caches: OpenSBI uses the
        // CSKY `csi_disable_data_preftch()` / `csi_flush_dcache_all()` /
        // `csi_disable_cache()` helpers (spacemit_k3.c L77-84); the
        // equivalent fences are emitted here as a best-effort approximation.
...
unsafe fn k3_pre_init(warmboot_addr: u64) {
    ...
    // matching OpenSBI's spacemit_k3.c which calls
    // cci_enable_snoop_dvm_reqs(0)..(6) directly on the CCI-550 slave
    // interfaces (bus-cci-550.c SLAVE_IFACE_OFFSET covers 0-6).
...
pub unsafe fn early_init(cold_boot: bool, warmboot_addr: u64) {
    ...
    // [`cold_boot_allowed`] (ML2SETUP), matching OpenSBI.
}
...
///
/// Mirrors OpenSBI `spacemit_k3_emulate_load` (spacemit_k3.c L328-350):
...
///
/// Mirrors OpenSBI `spacemit_k3_emulate_store` (spacemit_k3.c L352-374).
...
/// `hartid < 8`).
pub unsafe fn suspend(hartid: usize, sleep_type: u32, state: &mut ImsicConfig) {
